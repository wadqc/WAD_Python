...
#from IQCpyModule.pyWAD_core import IQWADlib as wad
#from IQCpyModule.pyWAD_core import pydicom_series
...
def plot_rclist(rclist,order='az'):
...
class simple_nema_phantom():
    def __init__(self,data,mode,order=['x','y','z'],orientation='cw'):
        ...
        print data
        pixelsize,array,info = loaddata(data)
...
def update_wad(filename, mode, axorder, sporder,bgac, sphereac):
    ...
    output['z_slice'] = iqphantom.z
    output['z_slice_fig'] = zslicefig
    ...
    iqphantom.plot_dataspheres(iqphantom.z)
    ...
    inputcontrast = (sphereac - bgac)/ bgac
    meanrclist = {}
    maxrclist = {}
    ...
    plot_rclist(meanrclist,'za')
    plot_rclist(maxrclist,'za')
...
def main():
    ...
    #create_dcm(filename = filename, mode = m, axorder = order, sporder = ort,bgac = bga, sphereac = spa)
